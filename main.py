...
players = ["X","O"]
WIN = None
ttk.Label(root, text="Kulko i Kżyrzyk").place(in_=root,anchor="n",relx=.5)
informacje = ttk.Label(root, text="Kolej \"X\"")
informacje.place(in_=root,anchor="n",relx=.5,y=16)
def checkIfWon():
    global Pola, WIN
    for _y in range(3):
        y = _y*3
...
class Pole():
    def __init__(self,i):
        self.claimedBy = ""
        self.button = Button(frm, text="", width=5,command= lambda: self.press(players[player_turn]))
